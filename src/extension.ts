...
export function activate(context: vscode.ExtensionContext) {
	...
	const onOpenInNewWindow = vscode.commands.registerCommand('preview.openInNewWindow', () => {
		const panel = PreviewPanel.createOrShow(context.extensionUri, true);
		// 处理 Webview 消息
		panel.webview.onDidReceiveMessage(
				(message: any) => {
						switch (message.command) {
							case 'openLink':
								PreviewPanel.currentPanel.sendUrl('123');
								return;
						}
				},
				undefined,
	...
	context.subscriptions.push(onOpen, onUrl, onBack, onForward, onRefresh, onResponsiveView, onScreenView, onOpenDevTools, onOpenInBrowser, onOpenInNewWindow);
...
class PreviewPanel {
	...
	public static createOrShow(extensionUri: vscode.Uri, createIs: Boolean) {
		// If we already have a panel, show it.
		if (PreviewPanel.currentPanel || createIs) {
	...
	public async sendUrl(sendUrl: string) {
		let url = "";
		if(!sendUrl) {
